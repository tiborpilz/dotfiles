...
*** Completion/Search
**** Corfu
I want to use corfu instead of company. This means I have to set up some stuff manually.
...
Kind-icon provides configurable icon or text-based completion prefixes based on
the ~:company-kind~ property that many completion backends use, just like corfu.
...
is completion style. A completion style is a back-end for completion and is used
from a front-end that provides a completion UI.
...
***** Configure Corfu (And COrfu-Doc) for EXWM

If you use Corfu in the minibuffer below an X11 window, the Corfu childframe is
displayed beneath the X11 window, rendering candidates invisible. In order to
keep all candidates visible in this scenario use the following code:
...
If you additionally use corfu-doc, then the same situation above also applies to
childframes created by corfu-doc. To fix analogously, use the following code:
...
There are more possible configurations, but I'm not shure whether they are
necessary.  X
* UI
** Theming
*** Doom Themes
...
* Utilities
** exwm
Turns out, emacs can be used as a fully featured window manager - using [[https://github.com/ch11ng/exwm][exwm]], short for
...
For now, I'm running the example config.
...
I will also need a desktop entry to start the emacs wm from my display manager

#+begin_src txt :tangle /usr/share/xsessions/emacs.desktop
[Desktop Entry]
Name=Emacs
Exec=emacs
Type=Application
...
I'm using the default keybindings for now, but I will probably change them to be
more vim-like.

One thing to note is that not all commands will reach the X windows, since they
are caught by EXWM and noth passed through to the program. These keys are stored
in ~exwm-input-prefix-keys~. To adjust the global commands, EXWM will need to be restarted.

Multi-Monitor-Setup

Using more than one screen necessitates the usage of exwm-randr.
...
Usually, the buffer names are not very useful, so I'm renaming each buffer to the respective class name.
#+begin_src elisp
